...
def pretty_time_delta(seconds):
    ...
    if days > 0:
        return "{sign}{d} days {h} hours {m} minutes {s} seconds".format(
            sign=sign_string, d=days, h=hours, m=minutes, s=seconds
        )
    elif hours > 0:
        return "{sign}{h} hours {m} minutes {s} seconds".format(
            sign=sign_string, h=hours, m=minutes, s=seconds
        )
    elif minutes > 0:
